...
// Diff Mine 2
  ...
  diff = max(dot(normal, vec4(lightDirection, 1.0f)), 0.0f);
  diffuseColor += 30.0f * diff * mineColor / pow(dist, 2.25f);
// Spec
  reflectDir = reflect(-lightDirection, vec3(normal));
  spec = pow(max(dot(viewDir, reflectDir), 0.0f), 256);
  specularColor += specularStrength * spec * lightColor;
...
// Spec
  reflectDir = reflect(-lightDirection, vec3(normal));
  spec = pow(max(dot(viewDir, reflectDir), 0.0f), 16);
  specularColor += specularStrength * spec * lightColor;
